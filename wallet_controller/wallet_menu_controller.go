...
func SendTransaction(tx transaction.Transaction) {
	var connection networking.Connection
	isEstablished := connection.Establish()

	if isEstablished {
		var isAccepted bool = false
		connection.SendTransaction(tx, &isAccepted)
		if isAccepted {
...
func (wc *WalletController) handleInput(input string) {
	if input == "0" { // Update balance
	...
	} else if input == "1" { // Send coins
		var newTx transaction.Transaction
		var isCreated string
		for true {
			wc.ClearConsole()
			println("Select an operation. Type \"back\" to back to the main menu")
			println("0. Create a new transaction.")
			println("1. Sign an existing transaction.")

			wc.scann.Scan()
			btn := wc.scann.Text()
			if btn == "back" {
				return
			} else if btn == "0" { // Create transaction
				newTx, isCreated = wc.createTransaction()
				break
			} else if btn == "1" { // Sign tx
				newTx, isCreated = wc.signTransaction()
				break
			}
		}

		if isCreated == "" {
			go SendTransaction(newTx)
		...
		wc.ShowMyAddresses()
	} else if input == "5" { // Choose the other account
		menuLaunched = false
